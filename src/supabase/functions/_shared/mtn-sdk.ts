...
async function createApiUser(subscriptionKey: string, callbackHost: string): Promise<string> {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      if (attempt > 0) {
        console.log(`[MTN] Retry attempt ${attempt}/${maxRetries} for createApiUser...`);
        // Wait a bit between retries
        await new Promise(resolve => setTimeout(resolve, 1000 * attempt));
      }

      console.log(`[MTN] Creating API User (Attempt ${attempt + 1}) with ID: ${apiUser}`);
      ...
      console.error(`[MTN] Failed to create API User (Attempt ${attempt + 1}). Status: ${response.status}, Body: "${errorText}"`);
      lastError = new Error(`Failed to create API User: ${response.status} - ${errorText || 'Empty response'}`);
      
      // If it's a 500 error, we should definitely retry. If it's 401/400, retry might not help but we'll try anyway.
      if (response.status !== 500 && response.status !== 503 && response.status !== 504) {
        break; // Don't retry on client errors like 400, 401, 403
      }
    } catch (error) {
      console.error(`[MTN] Exception creating API User (Attempt ${attempt + 1}):`, error);
      lastError = error instanceof Error ? error : new Error(String(error));
    }
  }

  throw lastError || new Error('Failed to create API User after retries');
}
...
async function getApiCredentials(
  ...
  // Sanitize callback host - ensure it is a valid hostname/URL format
  const rawCallbackHost = Deno.env.get('MTN_CALLBACK_HOST') || 'webhook.site';
  let callbackHost = rawCallbackHost.replace(/\s+/g, '');
  
  // For sandbox, many guides recommend a clean hostname or localhost
  // If it starts with http:// or https://, keep it, otherwise it's just a hostname
  // But if it's failing with 500, let's try to ensure it's a valid format
  if (callbackHost.includes('://')) {
    try {
      const url = new URL(callbackHost);
      callbackHost = url.hostname;
    } catch {
      // If not a valid URL, just use as is but trimmed
    }
  }
