...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { amount, phone_number }: DepositRequest = await req.json();
    ...
    try {
      // Call MTN SDK to initiate payment request
      ...
      return new Response(
        JSON.stringify({ error: 'Erreur lors de la demande de paiement MTN' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
  } catch (error) {
