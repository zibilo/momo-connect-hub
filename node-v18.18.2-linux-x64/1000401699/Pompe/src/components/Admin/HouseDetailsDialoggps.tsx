...
import { 
  ...
  Navigation, Map as MapIcon // Import des nouvelles icônes
} from "lucide-react";
...
export function HouseDetailsDialog({ house, open, onOpenChange }: HouseDetailsDialogProps) {
  ...
  // Au montage ou changement de maison, on lance la recherche GPS
  useEffect(() => {
    if (open) {
      setCoordinates(null);
      fetchCoordinates();
  ...
  // --- LOGIQUE DE GÉOCODAGE INTELLIGENTE ---
  const fetchCoordinates = async () => {
    ...
    try {
      ...
      // TENTATIVE 1 : Précision maximale (Ruelle + Quartier + Ville)
      if (street && neighborhood && city) {
        const query = `${street}, ${neighborhood}, ${city}, République du Congo`;
        console.log("Géocodage (Précis) :", query);
        const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&limit=1`);
        data = await res.json();
      }

      // TENTATIVE 2 : Quartier + Ville (Si tentative 1 échoue ou pas de rue)
      if ((!data || data.length === 0) && neighborhood && city) {
        console.warn("Adresse précise non trouvée. Essai Quartier...");
        const query = `${neighborhood}, ${city}, République du Congo`;
        const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&limit=1`);
        data = await res.json();
      }

      // TENTATIVE 3 : Ville uniquement (Dernier recours)
      if ((!data || data.length === 0) && city) {
        console.warn("Quartier non trouvé. Essai Ville...");
      ...
      if (data && data.length > 0) {
        ...
        toast.success("Localisation GPS trouvée");
      } else {
        console.warn("Aucun résultat de géocodage trouvé.");
      }
    } catch (error) {
      console.error("Erreur technique géocodage:", error);
    } finally {
      setGeoLoading(false);
    }
  };

  // Lancer Google Maps / Waze
  const startNavigation = () => {
    if (!coordinates) {
      toast.error("Coordonnées GPS introuvables.");
      return;
    }
    const url = `https://www.google.com/maps/dir/?api=1&destination=${coordinates.lat},${coordinates.lon}`;
  ...
  const handleDownloadPlan = async () => {
    ...
    try {
      ...
      try {
        ...
        const { data, error } = await supabase.storage.from('house-plans').download(path);
        ...
        link.click();
        document.body.removeChild(link);
        toast.success("Téléchargement réussi via API");
      } catch (err) {
  ...
  const handleAnalyzePlan = async (mode: 'standard' | 'operational' = 'standard') => {
    ...
    setAnalyzing(true);
    toast.info(mode === 'operational' ? "Génération du rapport..." : "Analyse en cours...");
            ...
            <TabsContent value="info" className="space-y-6 mt-4">
              ...
              <Card className="p-4 border-l-4 border-l-red-600 bg-red-50/50">
                <div className="flex justify-between items-start mb-4">
                  ...
                  {coordinates ? (
                     <Badge variant="outline" className="bg-green-100 text-green-800 border-green-200">
                        <MapPin className="w-3 h-3 mr-1" /> GPS Localisé
                     </Badge>
                  ) : (
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  >
                    <Navigation className="mr-2 h-4 w-4" /> Lancer Navigation
                  </Button>
                  <Button 
                    onClick={openInMaps} 
                    variant="outline" 
                ...
                {!coordinates && !geoLoading && (
                  <p className="text-xs text-red-500 mt-2 text-center">
                    * Adresse non géolocalisée. Essayez de vérifier l'orthographe des lieux.
                  </p>
                )}
              </Card>
                ...
                <Card className="p-4 border-l-4 border-l-blue-500">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {(house.property_type === 'apartment' || house.building_name) && (
                      <>
                        <InfoRow icon={Building} label="Bâtiment" value={house.building_name} />
                        <InfoRow icon={Building} label="Étage" value={`${house.floor_number || 0} / ${house.total_floors || house.number_of_floors || '?'}`} />
                      </>
                    )}
                  </div>
                </Card>
              ...
              {operationalReport ? (
                    ...
                    <Card className="p-4 border-t-4 border-t-slate-600">
                      ...
                      <div className="bg-slate-50 p-3 rounded text-sm text-slate-700 space-y-2">
                        ...
                        <div className="mt-4 pt-2 border-t border-slate-200">
                          <p className="font-bold text-xs mb-1">DONNÉES COMPLÉMENTAIRES :</p>
                ...
                <Card className="p-12 text-center flex flex-col items-center border-2 border-dashed border-slate-300">
                  ...
                  <p className="text-muted-foreground max-w-md mt-2 mb-6">
                    Générez un rapport tactique spécifique pour les équipes d'intervention.
                  </p>
                  <Button 
                    onClick={() => handleAnalyzePlan('operational')} 
                    disabled={analyzing || !house.plan_url}
