...
import { 
  ...
  CreditCard, Navigation, Map as MapIcon, ExternalLink
} from "lucide-react";
...
export function HouseDetailsDialog({ house, open, onOpenChange }: HouseDetailsDialogProps) {
  ...
  // Séparation des CNI et des autres documents
  ...
  // --- GÉOLOCALISATION ---
  // Construction de l'adresse complète pour la recherche
  ...
  // URLs des services de cartes
  const googleMapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodedAddress}`;
  const organicMapsUrl = `https://www.openstreetmap.org/search?query=${encodedAddress}`;

  // Helper pour l'affichage des lignes d'infos
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col">
        ...
        <div className="flex-1 mt-2">
          <Tabs defaultValue="info" className="w-full">
            ...
            {/* BARRE D'ONGLETS (Analyse IA retirée) */}
            ...
            <TabsContent value="location" className="mt-4 space-y-6">
              <Card className="p-6 border-blue-500/20 bg-blue-50/30 dark:bg-blue-950/10">
                <div className="flex flex-col md:flex-row gap-6 items-start">
                   ...
                   <div className="flex flex-col gap-3 w-full md:w-72 pt-2">
                      ...
                      <Button 
                        variant="outline" 
                        className="w-full border-green-600 text-green-700 hover:bg-green-50 shadow-sm"
                        onClick={() => window.open(organicMapsUrl, '_blank')}
                      >
              ...
              <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 flex gap-3">
                 ...
                 <p className="text-sm text-yellow-800 dark:text-yellow-200">
                   Les boutons ci-dessus ouvrent une recherche externe basée sur l'adresse fournie. La précision dépend du référencement des rues et quartiers dans ces services.
                 </p>
              </div>
            </TabsContent>
